...
exports.adjustToWorkingHour = adjustToWorkingHour;
...
function adjustToWorkingHour(date, holidays) {
    let adjusted = date;
    ...
    if (hour < 8)
        return adjusted.set({ hour: 8, minute: 0 });
    if (hour >= 12 && hour < 13)
        return adjusted.set({ hour: 13, minute: 0 });
    if (hour >= 17)
        return adjusted.plus({ days: 1 }).set({ hour: 8, minute: 0 });
    return adjusted;
}
...
function addWorkingHours(date, hours, holidays) {
    let current = date;
    let remaining = hours;
    while (remaining > 0) {
        if (!isWorkingDay(current, holidays)) {
